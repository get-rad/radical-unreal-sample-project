...
#define LOCTEXT_NAMESPACE "RadicalLiveLinkSource"

#define RECV_BUFFER_SIZE 1024 * 1024

FRadicalLiveLinkSource::FRadicalLiveLinkSource(FIPv4Endpoint InEndpoint)
: Socket(nullptr)
, Stopping(false)
, Thread(nullptr)
, WaitTime(FTimespan::FromMilliseconds(100))
{
	...
	SourceType = LOCTEXT("RadicalLiveLinkSourceType", "Radical LiveLink");
	SourceMachineName = LOCTEXT("RadicalLiveLinkSourceMachineName", "wss");
...
FRadicalLiveLinkSource::~FRadicalLiveLinkSource()
{
	Stop();
	if (Thread != nullptr)
	{
...
void FRadicalLiveLinkSource::ReceiveClient(ILiveLinkClient* InClient, FGuid InSourceGuid)
{
	Client = InClient;
...
bool FRadicalLiveLinkSource::IsSourceStillValid() const
{
...
bool FRadicalLiveLinkSource::RequestSourceShutdown()
{
	Stop();
...
void FRadicalLiveLinkSource::Start()
{
	ThreadName = "Radical UDP Receiver ";
	ThreadName.AppendInt(FAsyncThreadIndex::GetNext());
	
	Thread = FRunnableThread::Create(this, *ThreadName, 128 * 1024, TPri_AboveNormal, FPlatformAffinity::GetPoolThreadMask());
}

void FRadicalLiveLinkSource::Stop()
{
	Stopping = true;
}

uint32 FRadicalLiveLinkSource::Run()
{
...
void FRadicalLiveLinkSource::HandleReceivedData(TSharedPtr<TArray<uint8>, ESPMode::ThreadSafe> ReceivedData)
{
	FString JsonString;
